...
class Rules:
    ...
    def rule6(self,cell,u,c):   # spiking dendritic segments potentially learn
        S = cell.syn.S(c,cell.P)
        cell.L = cell._L = cell.syn.L(c,S)      # learning deltas
...
class Cyron:
    ...
    def iter(self,u,c):                # cell algo phase `ph`
        self.p = self._p               # cyclo state transition
        self.P = self._P.copy()        # permanence matrix state transition
        self.L = self._L.copy()        # learning matrix state transition
        if self.p == 0:                # BASE state
            c = self.rule0(u,c) # a burst state is transient
            c = self.rule1(u,c) # excited predictive cells get active
            self._p = 1                # advance to PRE state
        elif self.p == 1:              # PRE state
            c = self.rule2(u,c) # excited neurons in non-predictive groups burst
            self._p = 2                # advance to POST state
        elif self.p == 2:              # READY state (all outputs are settled)
            c = self.rule3(u,c) # excited bursting neurons get active
            self._p = 3                # advance to POST state
        elif self.p == 3:              # POST state
            c = self.rule4(u,c) # spiking dentrites of active neurons learn
            c = self.rule5(u,c) # spiking neurons get always predictive
            c = self.rule6(u,c) # empowered dendritic segments spike
            self._p = 0                # back to BASE state
        else:
            raise Exception("bad cyclo state")
        return c
